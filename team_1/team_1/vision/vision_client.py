...
class VisionClient(Node):
    def __init__(self):
        ...
        self.latest_grasp_stamped: PoseStamped | None = None
        self.latest_grasp_base_stamped: PoseStamped | None = None
        self.latest_detection_json: str = ""
        self.latest_grasp_json: str = ""
        self.create_subscription(PoseStamped, "/vision/selected_pose", self._pose_cb, 10)
        self.create_subscription(PoseStamped, "/vision/selected_grasp", self._grasp_cb, 10)
        ...
        self.create_subscription(String, "/vision/grasp_candidates", self._grasp_json_cb, 10)
...
def main(args=None):
    ...
    if response is None:
        ...
        if node.latest_grasp_stamped is not None:
            ps = node.latest_grasp_stamped
            pp = ps.pose.position
            node.get_logger().info(
                f"selected_grasp frame={ps.header.frame_id!r} "
                f"xyz=({pp.x:.4f}, {pp.y:.4f}, {pp.z:.4f})"
            )
        if node.latest_grasp_base_stamped is not None:
            ps = node.latest_grasp_base_stamped
            pp = ps.pose.position
            node.get_logger().info(
                f"selected_grasp_base frame={ps.header.frame_id!r} "
                f"xyz=({pp.x:.4f}, {pp.y:.4f}, {pp.z:.4f})"
            )
        if node.latest_grasp_json:
            node.get_logger().info(f"grasp_candidates={node.latest_grasp_json[:500]}")
