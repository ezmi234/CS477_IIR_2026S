#!/usr/bin/env python3
import json
import os
import re
from dataclasses import asdict, dataclass
from datetime import datetime
from pathlib import Path

import cv2
import numpy as np
import rclpy
import sensor_msgs_py.point_cloud2 as pc2
import std_msgs.msg
from cv_bridge import CvBridge, CvBridgeError
from geometry_msgs.msg import Pose
from google import genai
from PIL import Image as PILImage
from rclpy.executors import ExternalShutdownException
from rclpy.node import Node
from rclpy.qos import DurabilityPolicy, QoSProfile, ReliabilityPolicy
from riro_srvs.srv import StringPose
from sensor_msgs.msg import Image, PointCloud2, PointField
from std_msgs.msg import String

try:
    from ultralytics import YOLO
except ImportError:
    YOLO = None


DEFAULT_MODEL_PATH = Path(__file__).resolve().parent / 'model' / 'best.pt'
DEFAULT_SAVE_DIR = Path.home() / 'cs477_ws' / 'team_0_detection_results'


@dataclass(frozen=True)
class ObjectModel:
    name: str
    aliases: tuple
    depth_margin_m: float

    def to_dict(self):
        data = asdict(self)
        data['aliases'] = list(self.aliases)
        return data


OBJECT_MODELS = {
    'banana': ObjectModel('banana', ('banana',), 0.05),
    'coke_can': ObjectModel('coke_can', ('coke can', 'coke', 'cola', 'can'), 0.05),
    'hammer': ObjectModel('hammer', ('hammer',), 0.08),
    'meat_can': ObjectModel('meat_can', ('meat can', 'meat', 'spam'), 0.055),
    'strawberry': ObjectModel('strawberry', ('strawberry',), 0.035),
}


@dataclass
class Detection:
    label: str
    confidence: float
    left: int
    top: int
    right: int
    bottom: int

    @property
    def center(self):
        return (self.left + self.right) // 2, (self.top + self.bottom) // 2

    @property
    def xyxy(self):
        return self.left, self.top, self.right, self.bottom

    def padded(self, image_shape, ratio):
        image_h, image_w = image_shape[:2]
        width = self.right - self.left
        height = self.bottom - self.top
        pad_x = int(round(width * ratio))
        pad_y = int(round(height * ratio))
        return Detection(
            label=self.label,
            confidence=self.confidence,
            left=max(0, self.left - pad_x),
            top=max(0, self.top - pad_y),
            right=min(image_w - 1, self.right + pad_x),
            bottom=min(image_h - 1, self.bottom + pad_y),
        )

    def to_dict(self):
        return {
            'label': self.label,
            'confidence': float(self.confidence),
            'bbox_xyxy': [int(self.left), int(self.top), int(self.right), int(self.bottom)],
            'center_xy': [int(self.center[0]), int(self.center[1])],
        }


def normalize_label(label):
    return str(label or '').lower().replace('_', ' ').replace('-', ' ').strip()


def get_model(label):
    normalized = normalize_label(label)
    if not normalized:
        return None

    for model in OBJECT_MODELS.values():
        names = (model.name, *model.aliases)
        if any(normalized == normalize_label(name) for name in names):
            return model

    for model in OBJECT_MODELS.values():
        names = (model.name, *model.aliases)
        if any(normalized in normalize_label(name) or normalize_label(name) in normalized
               for name in names):
            return model
    return None


def known_labels():
    return sorted(OBJECT_MODELS.keys())


def parse_target_label(text):
    normalized_text = normalize_label(text)
    if not normalized_text:
        return ''

    direct_model = get_model(normalized_text)
    if direct_model is not None:
        return direct_model.name

    for model in OBJECT_MODELS.values():
        for name in (model.name, *model.aliases):
            normalized_name = normalize_label(name)
            if normalized_name and normalized_name in normalized_text:
                return model.name
    return normalized_text


def image_msg_to_cv2_fallback(msg, desired_encoding=None):
    encoding = str(msg.encoding or '').lower()
    specs = {
        'rgb8': (np.uint8, 3),
        'bgr8': (np.uint8, 3),
        'rgba8': (np.uint8, 4),
        'bgra8': (np.uint8, 4),
        'mono8': (np.uint8, 1),
        '8uc1': (np.uint8, 1),
        '8uc3': (np.uint8, 3),
        '8uc4': (np.uint8, 4),
        'mono16': (np.uint16, 1),
        '16uc1': (np.uint16, 1),
        '32fc1': (np.float32, 1),
    }
    if encoding not in specs:
        raise ValueError(f"unsupported image encoding '{msg.encoding}'")

    dtype, channels = specs[encoding]
    itemsize = np.dtype(dtype).itemsize
    min_step = int(msg.width) * channels * itemsize
    row_step = int(msg.step) if int(msg.step) > 0 else min_step
    required = row_step * int(msg.height)
    if len(msg.data) < required:
        raise ValueError(f'image data too short: got={len(msg.data)}, need={required}')

    flat = np.frombuffer(msg.data, dtype=dtype, count=required // itemsize)
    rows = flat.reshape(int(msg.height), row_step // itemsize)
    pixels = rows[:, : min_step // itemsize]
    if channels == 1:
        image = pixels.reshape(int(msg.height), int(msg.width)).copy()
    else:
        image = pixels.reshape(int(msg.height), int(msg.width), channels).copy()

    if desired_encoding == 'bgr8':
        if encoding in ('rgb8', '8uc3'):
            return cv2.cvtColor(image, cv2.COLOR_RGB2BGR)
        if encoding == 'bgr8':
            return image
        if encoding == 'rgba8':
            return cv2.cvtColor(image, cv2.COLOR_RGBA2BGR)
        if encoding == 'bgra8':
            return cv2.cvtColor(image, cv2.COLOR_BGRA2BGR)
        if channels == 1 and image.dtype == np.uint8:
            return cv2.cvtColor(image, cv2.COLOR_GRAY2BGR)
        raise ValueError(f"cannot convert encoding '{msg.encoding}' to bgr8")

    return image


def point_bounds(points):
    if points is None or len(points) == 0:
        return {}
    points = np.asarray(points, dtype=np.float64)[:, :3]
    min_xyz = np.min(points, axis=0)
    max_xyz = np.max(points, axis=0)
    return {
        'min_xyz': min_xyz.tolist(),
        'max_xyz': max_xyz.tolist(),
        'extent_xyz': (max_xyz - min_xyz).tolist(),
    }


def make_depth_visual(depth_crop):
    if depth_crop is None or depth_crop.size == 0:
        return None
    depth = depth_crop[:, :, 0] if depth_crop.ndim == 3 else depth_crop
    depth = depth.astype(np.float32, copy=False)
    finite = np.isfinite(depth)
    if not np.any(finite):
        return np.zeros((*depth.shape[:2], 3), dtype=np.uint8)
    finite_depth = depth[finite]
    low = float(np.percentile(finite_depth, 2.0))
    high = float(np.percentile(finite_depth, 98.0))
    if high <= low:
        high = low + 1.0
    normalized = np.zeros(depth.shape[:2], dtype=np.uint8)
    clipped = np.clip(depth, low, high)
    normalized[finite] = ((clipped[finite] - low) * 255.0 / (high - low)).astype(np.uint8)
    return cv2.applyColorMap(normalized, cv2.COLORMAP_JET)


def select_center_component(mask):
    mask = mask.astype(np.uint8)
    num_labels, labels, stats, centroids = cv2.connectedComponentsWithStats(mask, connectivity=8)
    if num_labels <= 1:
        return mask.astype(bool)

    h, w = mask.shape[:2]
    center = np.asarray([(w - 1) * 0.5, (h - 1) * 0.5], dtype=np.float64)
    component_centers = centroids[1:]
    distances = np.linalg.norm(component_centers - center, axis=1)
    areas = stats[1:, cv2.CC_STAT_AREA].astype(np.float64)
    score = distances / np.sqrt(np.maximum(areas, 1.0))
    selected_label = int(np.argmin(score) + 1)
    return labels == selected_label


def estimate_center_depth(cloud_crop, valid_mask, window_ratio=0.25):
    h, w = valid_mask.shape[:2]
    half_h = max(2, int(round(h * window_ratio * 0.5)))
    half_w = max(2, int(round(w * window_ratio * 0.5)))
    cy = h // 2
    cx = w // 2
    y1 = max(0, cy - half_h)
    y2 = min(h, cy + half_h + 1)
    x1 = max(0, cx - half_w)
    x2 = min(w, cx + half_w + 1)
    center_valid = valid_mask[y1:y2, x1:x2]
    center_z = cloud_crop[y1:y2, x1:x2, 2][center_valid]
    if center_z.size >= 10:
        return float(np.median(center_z)), int(center_z.size)
    all_z = cloud_crop[:, :, 2][valid_mask]
    return float(np.median(all_z)), int(all_z.size)


def extract_rgbd_pose(rgb_image, depth_image, cloud, detection, depth_margin_m, min_points):
    if cloud is None or cloud.ndim != 3:
        raise ValueError('organized PointCloud2 with shape (H, W, 3) is required')

    image_h, image_w = rgb_image.shape[:2]
    cloud_h, cloud_w = cloud.shape[:2]
    left, top, right, bottom = detection.xyxy
    left = int(np.clip(left, 0, image_w - 1))
    right = int(np.clip(right, 0, image_w - 1))
    top = int(np.clip(top, 0, image_h - 1))
    bottom = int(np.clip(bottom, 0, image_h - 1))
    if right <= left or bottom <= top:
        raise ValueError(f'invalid bbox: {detection.xyxy}')

    scale_x = cloud_w / float(image_w)
    scale_y = cloud_h / float(image_h)
    cx1 = int(np.clip(round(left * scale_x), 0, cloud_w - 1))
    cx2 = int(np.clip(round(right * scale_x), 0, cloud_w - 1))
    cy1 = int(np.clip(round(top * scale_y), 0, cloud_h - 1))
    cy2 = int(np.clip(round(bottom * scale_y), 0, cloud_h - 1))
    if cx2 <= cx1 or cy2 <= cy1:
        raise ValueError(f'invalid cloud bbox: {(cx1, cy1, cx2, cy2)}')

    rgb_crop = rgb_image[top:bottom, left:right].copy()
    depth_crop = depth_image[top:bottom, left:right].copy() if depth_image is not None else None
    cloud_crop = cloud[cy1:cy2, cx1:cx2].copy()
    valid_mask = np.isfinite(cloud_crop).all(axis=2) & (cloud_crop[:, :, 2] > 0.0)
    if not np.any(valid_mask):
        raise ValueError('no valid depth points inside bbox')

    valid_z = cloud_crop[:, :, 2][valid_mask]
    z_min = float(np.min(valid_z))
    z_far = float(np.percentile(valid_z, 95.0))
    z_center, center_depth_points = estimate_center_depth(cloud_crop, valid_mask)
    half_band = float(depth_margin_m) * 0.5
    foreground_min = max(z_min, z_center - half_band)
    foreground_max = min(z_far, z_center + half_band)
    foreground_mask = (
        valid_mask
        & (cloud_crop[:, :, 2] >= foreground_min)
        & (cloud_crop[:, :, 2] <= foreground_max)
    )
    foreground_mask = select_center_component(foreground_mask)
    foreground_points = cloud_crop[foreground_mask][:, :3]

    if len(foreground_points) < min_points:
        relaxed_min = max(z_min, z_center - depth_margin_m)
        relaxed_max = min(z_far, z_center + depth_margin_m)
        foreground_mask = (
            valid_mask
            & (cloud_crop[:, :, 2] >= relaxed_min)
            & (cloud_crop[:, :, 2] <= relaxed_max)
        )
        foreground_mask = select_center_component(foreground_mask)
        foreground_points = cloud_crop[foreground_mask][:, :3]
        foreground_min = relaxed_min
        foreground_max = relaxed_max

    if len(foreground_points) < min_points:
        raise ValueError(f'only {len(foreground_points)} foreground points found; need {min_points}')

    centroid = np.median(foreground_points, axis=0)
    return {
        'rgb_crop': rgb_crop,
        'depth_crop': depth_crop,
        'cloud_crop': cloud_crop,
        'mask': foreground_mask.astype(np.uint8) * 255,
        'foreground_points': foreground_points,
        'centroid': centroid,
        'bbox_xyxy': (left, top, right, bottom),
        'cloud_bbox_xyxy': (cx1, cy1, cx2, cy2),
        'stats': {
            'z_min': z_min,
            'z_percentile_95': z_far,
            'z_center_median': z_center,
            'center_depth_points': center_depth_points,
            'foreground_z_min': foreground_min,
            'foreground_z_max': foreground_max,
            'foreground_points': int(len(foreground_points)),
            'foreground_bounds': point_bounds(foreground_points),
        },
    }


class YoloDetector:
    def __init__(self, model_path, confidence=0.35, iou=0.45):
        model_path = Path(model_path).expanduser()
        if YOLO is None:
            raise ImportError('ultralytics is not installed')
        if not model_path.is_file():
            raise FileNotFoundError(f'YOLO model file not found: {model_path}')
        self.model_path = model_path
        self.model = YOLO(str(model_path))
        self.confidence = float(confidence)
        self.iou = float(iou)
        self.class_names = self.model.names

    def detect(self, image, _prompt_text=''):
        result = self.model.predict(source=image, conf=self.confidence, iou=self.iou, verbose=False)[0]
        detections = []
        if result.boxes is None:
            return detections

        image_h, image_w = image.shape[:2]
        for box in result.boxes:
            left, top, right, bottom = box.xyxy[0].detach().cpu().numpy().astype(int).tolist()
            left = int(np.clip(left, 0, image_w - 1))
            right = int(np.clip(right, 0, image_w - 1))
            top = int(np.clip(top, 0, image_h - 1))
            bottom = int(np.clip(bottom, 0, image_h - 1))
            if right <= left or bottom <= top:
                continue
            cls_id = int(box.cls[0].detach().cpu().item())
            label = str(self.class_names.get(cls_id, cls_id))
            confidence = float(box.conf[0].detach().cpu().item())
            detections.append(Detection(label, confidence, left, top, right, bottom))
        detections.sort(key=lambda det: det.confidence, reverse=True)
        return detections


class GeminiDetector:
    def __init__(self, api_key, model='gemini-2.5-flash'):
        if not api_key:
            raise ValueError('GEMINI_API_KEY is not set')
        self.client = genai.Client(api_key=api_key)
        self.model = model

    def detect(self, image, prompt_text=''):
        rgb_img = cv2.cvtColor(image, cv2.COLOR_BGR2RGB)
        pil_img = PILImage.fromarray(rgb_img)
        prompt = prompt_text or 'Detect objects and return [ymin, xmin, ymax, xmax, label]'
        format_instruction = (
            "\n\nOutput format: [ymin, xmin, ymax, xmax, 'label'] "
            'using normalized coordinates (0-1000). Return all matching objects. '
            'Only return the list(s), no other text.'
        )
        response = self.client.models.generate_content(
            model=self.model,
            contents=[pil_img, prompt + format_instruction],
        )
        text = response.text or ''
        pattern = r'\[(\d+),\s*(\d+),\s*(\d+),\s*(\d+),\s*["\']?([\w\s_ -]+)["\']?\]'
        matches = re.findall(pattern, text)
        h, w = image.shape[:2]
        detections = []
        for match in matches:
            ymin, xmin, ymax, xmax, label = match
            left = int(np.clip(int(xmin) * w / 1000.0, 0, w - 1))
            right = int(np.clip(int(xmax) * w / 1000.0, 0, w - 1))
            top = int(np.clip(int(ymin) * h / 1000.0, 0, h - 1))
            bottom = int(np.clip(int(ymax) * h / 1000.0, 0, h - 1))
            if right <= left or bottom <= top:
                continue
            detections.append(Detection(label.strip(), 1.0, left, top, right, bottom))
        return detections


def select_detection(detections, target_label):
    if not detections:
        return None
    model = get_model(target_label)
    target_names = [target_label]
    if model is not None:
        target_names = [model.name, *model.aliases]

    normalized_targets = [normalize_label(name) for name in target_names if normalize_label(name)]
    for detection in detections:
        if normalize_label(detection.label) in normalized_targets:
            return detection
    for detection in detections:
        normalized_detection = normalize_label(detection.label)
        if any(target in normalized_detection or normalized_detection in target
               for target in normalized_targets):
            return detection
    return None


class RgbdDetectionPoseServer(Node):
    def __init__(self):
        super().__init__('team_0_rgbd_detection_pose_server')

        self.declare_parameter('service_name', 'detect_objects_with_prompt')
        self.declare_parameter('backend', 'auto')
        self.declare_parameter('model_path', str(DEFAULT_MODEL_PATH))
        self.declare_parameter('confidence', 0.35)
        self.declare_parameter('iou', 0.45)
        self.declare_parameter('gemini_model', 'gemini-2.5-flash')
        self.declare_parameter('image_topic', '/wrist_camera/wrist_camera/color/image_raw')
        self.declare_parameter('depth_topic', '/wrist_camera/wrist_camera/depth/color/image_raw')
        self.declare_parameter('points_topic', '/wrist_camera/wrist_camera/depth/color/points')
        self.declare_parameter('camera_frame', 'wrist_camera_color_optical_frame')
        self.declare_parameter('bbox_padding_ratio', 0.02)
        self.declare_parameter('min_roi_points', 30)
        self.declare_parameter('display', False)
        self.declare_parameter('save_dir', str(DEFAULT_SAVE_DIR))
        self.declare_parameter('annotated_image_topic', '/team_0/detection/annotated_image')
        self.declare_parameter('roi_mask_topic', '/team_0/detection/roi_mask')
        self.declare_parameter('roi_info_topic', '/team_0/detection/info')
        self.declare_parameter('roi_points_topic', '/team_0/detection/roi_points')

        self.bridge = CvBridge()
        self.latest_cv_img = None
        self.latest_depth_img = None
        self.latest_cloud = None
        self.annotated_img = None
        self.mask_img = None
        self.camera_frame = str(self.get_parameter('camera_frame').value)
        self.bbox_padding_ratio = float(self.get_parameter('bbox_padding_ratio').value)
        self.min_roi_points = int(self.get_parameter('min_roi_points').value)
        self.display = bool(self.get_parameter('display').value)
        self.save_dir = Path(self.get_parameter('save_dir').value).expanduser()
        self.save_dir.mkdir(parents=True, exist_ok=True)
        self.skip_counts = {}

        self.detector = self.create_detector()

        qos = QoSProfile(depth=10)
        qos.reliability = ReliabilityPolicy.BEST_EFFORT
        self.create_subscription(Image, self.get_parameter('image_topic').value, self.image_callback, qos)
        self.create_subscription(Image, self.get_parameter('depth_topic').value, self.depth_callback, qos)
        self.create_subscription(PointCloud2, self.get_parameter('points_topic').value, self.points_callback, qos)
        self.create_service(StringPose, self.get_parameter('service_name').value, self.detect_callback)

        latched_qos = QoSProfile(depth=10)
        latched_qos.reliability = ReliabilityPolicy.BEST_EFFORT
        latched_qos.durability = DurabilityPolicy.TRANSIENT_LOCAL
        self.annotated_image_pub = self.create_publisher(
            Image, self.get_parameter('annotated_image_topic').value, 10
        )
        self.roi_mask_pub = self.create_publisher(Image, self.get_parameter('roi_mask_topic').value, 10)
        self.roi_info_pub = self.create_publisher(String, self.get_parameter('roi_info_topic').value, 10)
        self.roi_points_pub = self.create_publisher(
            PointCloud2, self.get_parameter('roi_points_topic').value, latched_qos
        )
        self.timer = self.create_timer(0.2, self.timer_callback)
        self.get_logger().info(
            f'RGB-D detection pose server ready. backend={type(self.detector).__name__}, '
            f'known_labels={known_labels()}, save_dir={self.save_dir}'
        )

    def create_detector(self):
        backend = str(self.get_parameter('backend').value).strip().lower()
        model_path = self.get_parameter('model_path').value
        if backend in ('auto', 'yolo'):
            try:
                return YoloDetector(
                    model_path=model_path,
                    confidence=float(self.get_parameter('confidence').value),
                    iou=float(self.get_parameter('iou').value),
                )
            except Exception as exc:
                if backend == 'yolo':
                    raise
                self.get_logger().warn(f'YOLO backend unavailable, falling back to Gemini: {exc}')

        api_key = os.environ.get('GEMINI_API_KEY', '').strip()
        return GeminiDetector(api_key=api_key, model=self.get_parameter('gemini_model').value)

    def image_callback(self, msg):
        try:
            self.latest_cv_img = self.image_to_cv2(msg, desired_encoding='bgr8')
        except Exception as exc:
            self.log_skipped_message('rgb_convert', f'Skipping RGB image: {exc}')

    def depth_callback(self, msg):
        try:
            self.latest_depth_img = self.image_to_cv2(msg)
        except Exception as exc:
            self.log_skipped_message('depth_convert', f'Skipping depth image: {exc}')

    def points_callback(self, msg):
        if msg.width == 0 or msg.height == 0 or len(msg.data) == 0:
            self.log_skipped_message('points_empty', 'Skipping empty PointCloud2 message.')
            return
        raw_cloud = pc2.read_points_numpy(msg, field_names=('x', 'y', 'z'))
        self.latest_cloud = raw_cloud.reshape(msg.height, msg.width, 3) if msg.height > 1 else raw_cloud

    def image_to_cv2(self, msg, desired_encoding=None):
        if msg.width == 0 or msg.height == 0 or len(msg.data) == 0:
            raise ValueError('empty image message')
        try:
            return self.bridge.imgmsg_to_cv2(msg, desired_encoding=desired_encoding or 'passthrough')
        except CvBridgeError:
            return image_msg_to_cv2_fallback(msg, desired_encoding=desired_encoding)

    def log_skipped_message(self, key, message):
        count = self.skip_counts.get(key, 0) + 1
        self.skip_counts[key] = count
        if count == 1:
            self.get_logger().warn(f'{message} Further identical messages will be suppressed.')

    def timer_callback(self):
        if self.annotated_img is not None:
            msg = self.bridge.cv2_to_imgmsg(self.annotated_img, encoding='bgr8')
            msg.header.stamp = self.get_clock().now().to_msg()
            msg.header.frame_id = self.camera_frame
            self.annotated_image_pub.publish(msg)
            if self.display:
                cv2.imshow('Team 0 Detection', self.annotated_img)
                cv2.waitKey(1)
        if self.mask_img is not None:
            msg = self.bridge.cv2_to_imgmsg(self.mask_img, encoding='mono8')
            msg.header.stamp = self.get_clock().now().to_msg()
            msg.header.frame_id = self.camera_frame
            self.roi_mask_pub.publish(msg)

    def detect_callback(self, request, response):
        info = None
        try:
            info = self.detect_pose(request.data)
            pose = Pose()
            pose.position.x = float(info['location_xyz_m'][0])
            pose.position.y = float(info['location_xyz_m'][1])
            pose.position.z = float(info['location_xyz_m'][2])
            pose.orientation.w = 1.0
            response.pose = pose
        except Exception as exc:
            target = parse_target_label(request.data)
            info = {
                'ok': False,
                'target': target,
                'error': str(exc),
                'known_labels': known_labels(),
            }
            self.get_logger().warn(f'Detection failed: {exc}')
            pose = Pose()
            pose.orientation.w = 0.0
            response.pose = pose

        msg = String()
        msg.data = json.dumps(info, sort_keys=True)
        self.roi_info_pub.publish(msg)
        return response

    def detect_pose(self, request_text):
        if self.latest_cv_img is None or self.latest_depth_img is None or self.latest_cloud is None:
            missing = []
            if self.latest_cv_img is None:
                missing.append('RGB image')
            if self.latest_depth_img is None:
                missing.append('depth image')
            if self.latest_cloud is None:
                missing.append('organized point cloud')
            raise RuntimeError(f"{', '.join(missing)} has not been received yet")

        target_label = parse_target_label(request_text)
        target_model = get_model(target_label)
        if target_model is None:
            raise RuntimeError(f"Unknown target='{target_label}'. Choose one of: {known_labels()}")

        detections = self.detector.detect(self.latest_cv_img, request_text)
        selected = select_detection(detections, target_model.name)
        if selected is None:
            self.annotated_img = self.draw_debug_image(detections, None, None, None)
            debug_path = self.save_failure_debug_image(target_model.name)
            seen = [detection.label for detection in detections]
            raise RuntimeError(
                f"No detection matched target='{target_model.name}'. seen={seen}. "
                f"debug_image={debug_path}"
            )

        padded = selected.padded(self.latest_cv_img.shape, self.bbox_padding_ratio)
        roi = extract_rgbd_pose(
            rgb_image=self.latest_cv_img,
            depth_image=self.latest_depth_img,
            cloud=self.latest_cloud,
            detection=padded,
            depth_margin_m=target_model.depth_margin_m,
            min_points=self.min_roi_points,
        )
        request_dir = self.make_request_dir(target_model.name)
        files = self.save_detection_artifacts(
            request_dir, target_model, selected, padded, detections, roi, request_text
        )
        self.annotated_img = self.draw_debug_image(detections, selected, padded, roi)
        self.mask_img = roi['mask']
        self.publish_roi_cloud(roi['foreground_points'])

        info = {
            'ok': True,
            'target': target_model.name,
            'detected_label': selected.label,
            'frame_id': self.camera_frame,
            'detection': selected.to_dict(),
            'padded_detection': padded.to_dict(),
            'all_detections': [detection.to_dict() for detection in detections],
            'model': target_model.to_dict(),
            'roi': {
                'bbox_xyxy': [int(v) for v in roi['bbox_xyxy']],
                'cloud_bbox_xyxy': [int(v) for v in roi['cloud_bbox_xyxy']],
                'foreground_points': int(len(roi['foreground_points'])),
                'stats': roi['stats'],
            },
            'location_xyz_m': roi['centroid'].astype(float).tolist(),
            'save_dir': str(request_dir),
            'files': files,
        }
        self.get_logger().info(
            f"Detected {target_model.name}: bbox={selected.xyxy}, "
            f"xyz=({roi['centroid'][0]:.3f}, {roi['centroid'][1]:.3f}, {roi['centroid'][2]:.3f})"
        )
        return info

    def make_request_dir(self, label):
        timestamp = datetime.now().strftime('%Y%m%d_%H%M%S_%f')
        request_dir = self.save_dir / f'{timestamp}_{label}'
        request_dir.mkdir(parents=True, exist_ok=False)
        return request_dir

    def save_failure_debug_image(self, label):
        if self.annotated_img is None:
            return None
        debug_dir = self.save_dir / '_debug_failures'
        debug_dir.mkdir(parents=True, exist_ok=True)
        timestamp = datetime.now().strftime('%Y%m%d_%H%M%S_%f')
        debug_path = debug_dir / f'{timestamp}_{label}_detections.png'
        cv2.imwrite(str(debug_path), self.annotated_img)
        return str(debug_path)

    def save_detection_artifacts(self, request_dir, model, selected, padded, detections, roi, request_text):
        paths = {
            'rgb': request_dir / 'rgb.png',
            'depth_npy': request_dir / 'depth.npy',
            'depth_visualization': request_dir / 'depth_vis.png',
            'mask': request_dir / 'mask.png',
            'cloud_npy': request_dir / 'cloud.npy',
            'foreground_points_npy': request_dir / 'foreground_points.npy',
            'annotated': request_dir / 'annotated.png',
            'metadata': request_dir / 'metadata.json',
        }
        cv2.imwrite(str(paths['rgb']), roi['rgb_crop'])
        if roi['depth_crop'] is not None:
            np.save(paths['depth_npy'], roi['depth_crop'])
            depth_vis = make_depth_visual(roi['depth_crop'])
            if depth_vis is not None:
                cv2.imwrite(str(paths['depth_visualization']), depth_vis)
        cv2.imwrite(str(paths['mask']), roi['mask'])
        np.save(paths['cloud_npy'], roi['cloud_crop'])
        np.save(paths['foreground_points_npy'], roi['foreground_points'])
        cv2.imwrite(str(paths['annotated']), self.draw_debug_image(detections, selected, padded, roi))

        metadata = {
            'label': model.name,
            'request': request_text,
            'frame_id': self.camera_frame,
            'detection': selected.to_dict(),
            'padded_detection': padded.to_dict(),
            'all_detections': [detection.to_dict() for detection in detections],
            'location_xyz_m': roi['centroid'].astype(float).tolist(),
            'roi': {
                'bbox_xyxy': [int(v) for v in roi['bbox_xyxy']],
                'cloud_bbox_xyxy': [int(v) for v in roi['cloud_bbox_xyxy']],
                'stats': roi['stats'],
            },
        }
        paths['metadata'].write_text(json.dumps(metadata, indent=2, sort_keys=True), encoding='utf-8')
        return {key: str(path) for key, path in paths.items() if path.exists()}

    def draw_debug_image(self, detections, selected=None, padded=None, roi=None):
        image = self.latest_cv_img.copy()
        selected_xyxy = selected.xyxy if selected is not None else None
        for detection in detections:
            color = (0, 255, 255) if detection.xyxy == selected_xyxy else (0, 255, 0)
            cv2.rectangle(image, (detection.left, detection.top), (detection.right, detection.bottom), color, 2)
            cv2.circle(image, detection.center, 4, (0, 0, 255), -1)
            text_y = max(detection.top - 8, 18)
            cv2.putText(
                image,
                f'{detection.label} {detection.confidence:.2f}',
                (detection.left, text_y),
                cv2.FONT_HERSHEY_SIMPLEX,
                0.55,
                color,
                2,
            )

        if padded is not None:
            cv2.rectangle(image, (padded.left, padded.top), (padded.right, padded.bottom), (255, 128, 0), 2)

        if roi is not None:
            x1, y1, x2, y2 = roi['bbox_xyxy']
            if x2 > x1 and y2 > y1:
                mask = cv2.resize(roi['mask'], (x2 - x1, y2 - y1), interpolation=cv2.INTER_NEAREST)
                overlay = np.zeros((y2 - y1, x2 - x1, 3), dtype=np.uint8)
                overlay[:, :, 1] = mask
                region = image[y1:y2, x1:x2]
                if region.size and overlay.shape == region.shape:
                    image[y1:y2, x1:x2] = cv2.addWeighted(region, 0.75, overlay, 0.25, 0.0)
        return image

    def publish_roi_cloud(self, valid_points):
        if valid_points is None or len(valid_points) == 0:
            return
        header = std_msgs.msg.Header()
        header.stamp = self.get_clock().now().to_msg()
        header.frame_id = self.camera_frame
        fields = [
            PointField(name='x', offset=0, datatype=PointField.FLOAT32, count=1),
            PointField(name='y', offset=4, datatype=PointField.FLOAT32, count=1),
            PointField(name='z', offset=8, datatype=PointField.FLOAT32, count=1),
        ]
        points_float32 = valid_points[:, :3].astype(np.float32)
        msg = PointCloud2(
            header=header,
            height=1,
            width=len(points_float32),
            is_dense=True,
            is_bigendian=False,
            fields=fields,
            point_step=12,
            row_step=12 * len(points_float32),
            data=points_float32.tobytes(),
        )
        self.roi_points_pub.publish(msg)


def main(args=None):
    rclpy.init(args=args)
    node = RgbdDetectionPoseServer()
    try:
        rclpy.spin(node)
    except (KeyboardInterrupt, ExternalShutdownException):
        pass
    finally:
        cv2.destroyAllWindows()
        node.destroy_node()
        try:
            rclpy.shutdown()
        except Exception:
            pass


if __name__ == '__main__':
    main()
